...
public class EarthGolemEntity extends AbstractHorseEntity implements GeoEntity {
    ...
    @Override
    public EarthGolemEntity createChild(ServerWorld world, PassiveEntity entity) {
        return ModEntities.EARTH_GOLEM.create(world);
    }
    ...
    private <T extends GeoAnimatable> PlayState predicate(AnimationState<T> event) {
        if (event.isMoving()) {
            event.getController().setAnimation(RawAnimation.begin().then("animation.earth_golem.walk", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }

        event.getController().setAnimation(RawAnimation.begin().then("animation.earth_golem.idle", Animation.LoopType.LOOP));
        return PlayState.CONTINUE;
    }
