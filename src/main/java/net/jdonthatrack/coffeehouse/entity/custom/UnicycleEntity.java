...
public class UnicycleEntity extends AbstractHorseEntity implements GeoEntity {
    ...
    private <T extends GeoAnimatable> PlayState predicate(AnimationState<T> event) {
        if (event.isMoving()) {
            event.getController().setAnimation(RawAnimation.begin().then("animation.unicycle.walk", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }

        event.getController().setAnimation(RawAnimation.begin().then("animation.unicycle.idle", Animation.LoopType.LOOP));
        return PlayState.CONTINUE;
    }
